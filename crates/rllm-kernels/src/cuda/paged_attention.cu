...
// ── INT8 PagedAttention (decode + prefill) ─────────────────────────────────
//
// Same online-softmax attention as the FP16 path, but the K/V cache is stored
// as symmetric int8 (one byte/scalar) written by `cache_write_i8`. Each cache
// element is dequantized on read: `k = (float)q_k * k_scale`, `v = (float)q_v *
// v_scale`, where `k_scale`/`v_scale` are the per-tensor scales carried by
// GpuKVCache. Mirrors the fp8 kernels (which dequantize via fp8_to_float),
// substituting a linear dequant. The query stays FP16.

__global__ void paged_attention_decode_i8_kernel(
    __half* __restrict__ output,
    const __half* __restrict__ query,
    ...
    for (int64_t b = 0; b < num_blocks_needed; b++) {
        ...
        for (int64_t offset = block_start; offset < block_end; offset++) {
            ...
            int64_t k_idx = ((physical_block * num_kv_heads + kv_head) * head_dim + tid)
                            * block_size + blk_off;
            float k_val = (float)key_cache[k_idx] * k_scale;
            ...
            int64_t v_idx = ((physical_block * num_kv_heads + kv_head) * head_dim + tid)
                            * block_size + blk_off;
            float v_val = (float)value_cache[v_idx] * v_scale;
            out_val = out_val * correction + v_val * expf(logit - max_logits);
        }
    }
...
__global__ void paged_attention_prefill_i8_kernel(
    __half* __restrict__ output,
    const __half* __restrict__ query,
    const int8_t* __restrict__ key_cache,
    const int8_t* __restrict__ value_cache,
...
int32_t rllm_paged_attention_decode_i8(
    __half* output,
    const __half* query,
    ...
    paged_attention_decode_i8_kernel<<<grid, threads, shared_mem, stream>>>(
        output, query, key_cache, value_cache, block_tables, seq_lens,
        num_seqs, num_q_heads, num_kv_heads, head_dim, block_size,
        max_num_blocks_per_seq, scale, k_scale, v_scale);
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) return static_cast<int32_t>(err);
    return 0;
}

int32_t rllm_paged_attention_decode_i8_sync(
    __half* output,
    const __half* query,
    const int8_t* key_cache,
    const int8_t* value_cache,
    ...
    int32_t rc = rllm_paged_attention_decode_i8(
        output, query, key_cache, value_cache, block_tables, seq_lens,
        num_seqs, num_q_heads, num_kv_heads, head_dim, block_size,
        max_num_blocks_per_seq, scale, k_scale, v_scale, 0);
    if (rc != 0) return rc;
    cudaError_t err = cudaDeviceSynchronize();
    if (err != cudaSuccess) return static_cast<int32_t>(err);
    return 0;
...
int32_t rllm_paged_attention_prefill_i8(
    __half* output,
    const __half* query,
    const int8_t* key_cache,
    const int8_t* value_cache,
    ...
    paged_attention_prefill_i8_kernel<<<grid, threads, shared_mem, stream>>>(
        output, query, key_cache, value_cache, block_tables, seq_lens,
        query_start_loc, num_seqs, num_tokens, num_q_heads, num_kv_heads,
        head_dim, block_size, max_num_blocks_per_seq, scale, k_scale, v_scale);
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) return static_cast<int32_t>(err);
    return 0;
}

int32_t rllm_paged_attention_prefill_i8_sync(
    __half* output,
    const __half* query,
    const int8_t* key_cache,
    const int8_t* value_cache,
    ...
    int32_t rc = rllm_paged_attention_prefill_i8(
        output, query, key_cache, value_cache, block_tables, seq_lens,
        query_start_loc, num_seqs, num_tokens, num_q_heads, num_kv_heads,
        head_dim, block_size, max_num_blocks_per_seq, scale, k_scale, v_scale, 0);
    if (rc != 0) return rc;
    cudaError_t err = cudaDeviceSynchronize();
    if (err != cudaSuccess) return static_cast<int32_t>(err);
    return 0;
